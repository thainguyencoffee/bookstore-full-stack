...
onMounted(async () => {
  ...
  const response = await axios.get("/bff/login-options")
  const opts = response.data as LoginOptionDto[]
  if (opts.length) {
...
<span>
  ...
  <button
      @click="login" :disabled="!selectedLoginExperience || user.current.value.isAuthenticated">
    Login
  </button>
</span>
<div
